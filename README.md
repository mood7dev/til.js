...
- 만약 객체를 1개씩 만든다면 즉, `객체리터럴로 생성` 면 객체명은 `Camel Case`로
...
```js
function studentInfor(_name, _age) {
  this.name = _name;
  this.age = _age;
}
const stundent_1 = new studentInfor("홍길동", 10);
const stsundent_2 = new studentInfor("둘리", 2000);
```
...
function Student(name, age) {
  ...
  // 일반 함수로 프로퍼티 정의 (this 정확히 동작)
  this.sayName = function () {
    console.log(this.name); // OK
  };

  // 화살표 함수는 this를 공유하기 때문에 객체 메서드에는 부적절 (사용 X)
  this.sayAge = function () {
    console.log(this.age); // OK
  };

  // 최신 메서드 문법을 사용하려면 prototype으로 정의
  // 이건 생성자 밖에서 처리합니다.
}

// 최신 메서드 문법 (prototype에 정의, 메서드로 인식됨)
...
- `객체 생성자 함수`, `클래스`의 화살표 함수에서의 `this는 객체`를 가르킨다.
...
### 15.4. 무지막지하게 배열을 다룹니다. 데이터 종류중 배열을 다룹니다.

- 일반적 데이터 형태 : `[{},{},{},{}]`
- 배열을 프로그래밍에서 많이 다루므로 `미리 함수를 제공`함(빌트인 함수).
- 주의 사항으로서 `절대로 원본을 훼손하지 마셔야 합니다`.
